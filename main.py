...
bot1 = telebot.TeleBot('5619197827:AAFTgUJy7C_0YKMgYoT-ZB8_se2FPf244xA')
...
banned_users = [
  5380685424, 5272676030, 731918546, 1772411051, 297820198, 5710190212,
  5657609486, 5828378741, 5825904477, 5380685424, 5877237216, 5509031238
]

storage = MemoryStorage()
bot = Bot('5619197827:AAFTgUJy7C_0YKMgYoT-ZB8_se2FPf244xA')
...
async def admin(message: types.Message, state: FSMContext):
  if message.from_user.id == 1807653203:
    ...
    await message.answer(f"{message.from_user.first_name}, вы авторизованы!",
                         reply_markup=ikb)
  else:
    pass
    return
...
async def sending(call: types.CallbackQuery, state: FSMContext):
  ...
  async def reseption(message: types.Message):
    ...
    await message.answer(
      f'{call.from_user.first_name}, вы уверены, что хотите начать рассылку',
      reply_markup=ikb)


@dp.callback_query_handler(lambda c: c.data == 'yes_send',
                           state=SendState.first)
...
@dp.callback_query_handler(lambda c: c.data == 'no_send',
                           state=SendState.first)
...
async def start_command(message: types.Message):
  ...
  ikb.add(item1)
  await message.answer(
    f'<b>Я ознакомлен с <a href="https://telegra.ph/Pravila-ispolzovaniya-12-13">правилами использования</a> и соглашаюсь с ними</b>',
...
async def agree(call: types.CallbackQuery):
  ...
  item3 = InlineKeyboardButton(text='Сообщить о мошенничестве🚨',
                               callback_data='scam')
  item4 = InlineKeyboardButton(text='Проверить пользователя👮🏻',
                               callback_data='user')
  ...
  await call.message.answer(f'<b>Добро пожаловать на {text}!</b>',
                            reply_markup=ikb,
                            parse_mode=ParseMode.HTML,
                            disable_web_page_preview=True)
...
@dp.callback_query_handler(lambda c: c.data == 'menu6',
                           state=Conversation.waiting_for_input)
...
@dp.callback_query_handler(lambda c: c.data == 'menu5',
                           state=Conversation.waiting_for_input)
async def menu5(call: types.CallbackQuery, state: FSMContext):
...
async def post(call: types.CallbackQuery, state: FSMContext):
  ...
  user_channel_status = await bot.get_chat_member(chat_id='@YouTubeBirz',
  ...
  if user_channel_status["status"] != 'left':
    if call.from_user.id in banned_users:
      ...
      await call.message.answer(
        'Отправьте мне объявление\n\n<b>Можно отправлять больше одного фото😎</b>',
        reply_markup=ikb,
        parse_mode='HTML')
  else:
    ikb = InlineKeyboardMarkup(row_width=1)
  ...
  async def handle_albums(message: types.Message, state: FSMContext):
    ...
    if user_channel_status["status"] != 'left':
      if call.from_user.id in banned_users:
        ...
        await message.answer(
          f'Объявление готово к публикации✅\n\n'
      ...
      ikb = InlineKeyboardMarkup(row_width=1)
      item1 = InlineKeyboardButton(text='Биржа',
                                   url='https://t.me/YouTubeBirz')
      ikb.add(item1)
...
async def yes(call: types.CallbackQuery, state: FSMContext):
  bot1.forward_message(-1001538332180, call.message.chat.id,
...
async def yes(call: types.CallbackQuery, state: FSMContext):
  ...
  if user_channel_status["status"] != 'left':
    if call.from_user.id in banned_users:
      ...
      item1 = InlineKeyboardButton(text='Выложить пост📝',
                                   callback_data='post1')
      item2 = InlineKeyboardButton(text='Профиль📱', callback_data='profile')
      item3 = InlineKeyboardButton(text='Сообщить о мошенничестве🚨',
                                   callback_data='scam')
...
async def user(call: types.CallbackQuery, state: FSMContext):
  ...
  if user_channel_status["status"] != 'left':
    if call.from_user.id in banned_users:
      ...
      await call.message.answer(
        'Введите информацию о пользователе (Username, ID, карта, номер, киви кошелек и т.д.)',
        reply_markup=ikb)
      ...
      async def ready(message: types.Message, state: FSMContext):
        ...
        if word in file1:
          await message.answer(
            ...
            'Не советую проводить сделки с данным человеком',
            parse_mode=ParseMode.HTML,
            reply_markup=ikb1)
        else:
          await message.answer(f'<b>Пользователь НЕ найден в базе❌</b>',
                               parse_mode=ParseMode.HTML,
                               reply_markup=ikb1)
        await state.finish()
...
async def yes(call: types.CallbackQuery, state: FSMContext):
  ...
  if user_channel_status["status"] != 'left':
    if call.from_user.id in banned_users:
      ...
      await call.message.answer(
        'Предоставьте свой ник, ник мошенника, доказательства обмана, а также любую другую информацию о мошеннике.\n'
        'Можно отправлять больше, чем 1 сообщение',
        reply_markup=ikb)
      ...
      async def scaam(message: types.Message, state: FSMContext):
        ...
        bot1.forward_message(1807653203, call.message.chat.id,
                             message.message_id)

        await message.answer('Отправил!', reply_markup=ikb)

      @dp.message_handler(content_types=['text', 'photo'],
                          state=ScamState.onen)
      async def scaam(message: types.Message, state: FSMContext):
        ikb = InlineKeyboardMarkup()
